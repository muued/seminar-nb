...
\paragraph{Interval Estimates}
An interval estimator represents an interval overlapping the real value with some probability.
The interval borders together with a \emph{confidence coefficient}, the probability, that the interval will overlap the real value, we call a \emph{confidence interval}.
An example for an interval estimator is the postulation ``We are 90\% confident, that less than 1\% of our products are defective''.

\subsection{Statistical Inference}
From a philosophical perspective, statistical induction, also called statistical inference, is the method of drawing conclusions from empiricism to theory.
Its counterpart is the deduction, applying general rules to empirical instances.
\begin{figure}[h]
	\center
	...
	\end{tikzpicture}
\end{figure}
We therefore see hypothesis testing as a mean of statistical induction.
Hypothesis testing describes the process of inferring from a sample whether or not to accept a certain statement about a population.
In doing so, that statement has to be a \emph{hypothesis}.
%
...
This result is not too surprising, if one looks at figure~\ref{fig:1}. The blue curve describes Paul's, the orange curve Franz's cumulative distribution function. The red lines represent the value $D_n$, the maximum distance between the respective assumed function and the gray empirical distribution function.
...
Franz's test however has $4-1-2=1$ degree of freedom only, as he deduced the mean and variance from the sample.
...
For $2.8 > 2.706$ holds, Franz's hypothesis is rejected.
...
One can however use the function nag\_frequency\_table (g01aec) to calculate the frequency table.
...
It differs from the NAG C version in taking a vastly reduced number of parameters, especially when used for the Goodness-of-Fit test (it can also be used to perform contingency table tests based on the test statistic $X^2$).
...
In a situation with large sample size the choice is not clear and it may varies from case to case.
\newpage
\nocite{*}
