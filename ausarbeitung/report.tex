...
\\
When testing whether or not the null hypothesis is true, three different answers are possible:
\begin{enumerate}
	\item $H_0$ is false
	\item $H_0$ is true
	\item $H_0$ is not rejected
\end{enumerate}
Here, the third answer is different from the second in making no statement about $H_0$ being true or false.
It implies that the sample does not allow a statement about the truth value of the hypothesis with a reasonable confidence.
In some literature answer three is also phrased as $H_0$ is accepted \cite{conover1980practical}.
\\
The set of points in the sample space leading to answer 1 is also called the critical region.
\\
...
We distinguish between two types of errors when choosing the incorrect answer:
\begin{description}
